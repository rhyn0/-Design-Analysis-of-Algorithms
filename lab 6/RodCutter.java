...
public class RodCutter{
  private ArrayList<Integer> rodPrices;
  ...
  public static void main(String[] args){
    ...
    try{
      sc = new Scanner(new FileInputStream(new File("rodOptTest.txt")));
    }
  ...
  private void breakDown(ArrayList<Integer> indices, int index){
    int rodMax = rodPrices.get(index), prevalue = 0;
    ...
    if (table.get(index) != rodPrices.get(index)){
      ...
      indices.set(index, 0);
      prevalue = indices.get(lIndex);
      indices.set(lIndex, prevalue + 1);
      prevalue = indices.get(rIndex);
      indices.set(rIndex, prevalue + 1);
    }
  }
